...
@Component({
  selector: 'app-internet-refund-history',
  templateUrl: './internet-refund-history.component.html',
  styleUrls: ['./internet-refund-history.component.scss']
})
export class InternetRefundHistoryComponent implements OnInit, OnDestroy {
  ...
  generateReport(reportType) {
    this.reportService
      .download(reportType, 'internet_refunds', this.refundLazyEvent)
      .pipe(takeUntil(this.onDestroy$))
      .subscribe(response => {
        // download file
        let blob;
        ...
        if (reportType === this.reportTypes.pdf) {
          ...
          filename =
            'internet_refunds' + moment().format('DD_MM_YYYY_HH_mm_s') + '.pdf'; // date_report_name.pdf
          ...
          filename =
            'internet_refunds' + moment().format('DD_MM_YYYY_HH_mm_s') + '.csv'; // date_report_name.pdf
        }
